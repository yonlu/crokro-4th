...
void clif_getareachar_unit( map_session_data* sd,struct block_list *bl ){
	...
	if( ud && ud->walktimer != INVALID_TIMER ){
		...
		// TODO check if removal was root cause of failing compilation
		if (bl->type != BL_STALL) {
...
/// Displays a stall board to target/area (ZC_STORE_ENTRY).
/// 0131 <owner id>.L <message>.80B
// void clif_showstallboard( map_session_data& sd, enum send_target target, struct block_list* tbl )
// {
// 	if( bl == nullptr ){
// 		bl = &sd.bl;
// 		target = AREA_WOS;
...
void clif_parse_PurchaseReq2(int fd, map_session_data* sd){
	...
	// MOOSE check correctness of this
...
void clif_stall_vending_list(map_session_data *sd, s_stall_data *st){
	...
	if(st->vended_id == sd->status.char_id)
		...
		p->myStall = 0;
		// MOOSE check if static_cast solves compilation issues
	p->expireTime = static_cast<int>((st->expire_time - time(NULL)) * 1000); //if 0 == unlimited on client..
	...
	for(int i = 0; i < st->vend_num ; i++){
		if(st->items_inventory[i].amount > 0){
			...
			// MOOSE remove this depending on PACKETVER?
